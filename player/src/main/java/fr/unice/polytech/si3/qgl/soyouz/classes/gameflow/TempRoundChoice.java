...
public class TempRoundChoice
{
     ...
     * @param action for sailor to perform (anyAction, null) (moveAction, null) or (moveAction,
     *               AnyAction).
     * @throws IllegalArgumentException when illegal arguments.
     */
    public void hireSailor(Marin sailor, Pair<Integer, Integer> pos, Pair<GameAction,
        @Nullable GameAction> action)
    {
        ...
        {
            var optEntity =
                usedOnBoardEntity.keySet().stream().filter(e -> e.getPosCoord().equals(pos)).findAny();
        ...
        if (vacantSailors.contains(sailor))
        {
            ...
            if (!sailor.getPos().equals(pos))
            {
                ...
                if (!isActionMove)
                {
                    throw new IllegalArgumentException("Incoherent sailor position (position " +
                        "changed but no moveAction)");
                }
                ...
                {
                    throw new IllegalArgumentException("Incoherent sailor position (position not " +
                        "coherent with moveAction)");
                }
            }
            ...
            if (entity != null)
            {
                ...
                sailorsJob.put(sailor, Pair.of(new PosOnShip(pos.getFirst(), pos.getSecond()),
                    action));
            }
            else
            {
                movedSailors.put(sailor, Pair.of(new PosOnShip(pos.getFirst(), pos.getSecond()),
                    (MoveAction) action.getFirst()));
            }
        }
        else if (movedSailors.containsKey(sailor))
        {
            ...
            sailorsJob.put(sailor, Pair.of(new PosOnShip(pos.getFirst(), pos.getSecond()),
                Pair.of(oldSailor.getSecond(), action.getFirst())));
            usedOnBoardEntity.replace(entity, true);
        }
        else
        {
     ...
     * @throws IllegalArgumentException when illegal arguments
     */
    public void hireSailor(Marin sailor, Pair<Integer, Integer> pos, GameAction act1,
                           @Nullable GameAction act2)
    {
